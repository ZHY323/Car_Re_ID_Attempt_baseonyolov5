...
def draw_bboxes(image, bboxes, line_thickness, rootpath, stat_dic):
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        #print(f'{x1}:{x2}:{y1}:{y2}:{cls_id}:{pos_id}')
        dic_key = '{}{}'.format(cls_id , pos_id)
        savepath = rootpath + '{}'.format(dic_key)
        stat_dic[dic_key] = stat_dic.get(dic_key,0) + 1
        cut = image[y1:y2, x1:x2]
        if not os.path.isdir(savepath):
            os.makedirs(savepath)
        try:
            cv2.imwrite('{}//{}.jpg'.format(savepath, stat_dic[dic_key]), cut)
        except:
            print('有一帧存在错误')
